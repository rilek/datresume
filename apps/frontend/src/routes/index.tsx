...
const EditorWrapper = () => {
  ...
  const fetchResume = useAppStore((state) => state.fetchLocalResume);
  const loading = useAppStore((state) => state.loading);
  const showDiff = useAppStore((state) => state.showDiff);
  const setContent = useAppStore((state) => state.setContent)("local");
  ...
  return <Editor content={resume.content.html || ""} loading={loading} compare={showDiff} onChange={setContent} />;
};
    ...
    <div className="max-w-6xl py-24 mx-auto px-4">
      <div className="max-w-2xl mx-auto flex flex-col justify-center">
        ...
        <p className="mt-8 mx-auto text-xl text-black/70 leading-8">
          Adjust your resume to any job offer in seconds with AI Chat. Just write or paste resume in the editor, send
          job URL or text to chat and let the magic happen.
        </p>
  ...
  return (
    <div id={editorAreaId}>
      <div className="flex items-start mb-12 mx-4 gap-4 max-w-screen">
        ...
        <div
          className={clsx(
            "animate-in text-sm slide-in-from-right transition-all border-gray-200 sticky top-4 xl:w-md w-xs",
            {
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button onClick={resetChat} size="icon-sm" variant={"ghost"}>
                      <ListXIcon />
                    </Button>
                  </TooltipTrigger>
            ...
            <div className="flex flex-col gap-2 mt-8">
              {chatMessages.length === 0 ? (
                ...
                <>
                  ...
                  <div className="mt-8">
                    <ChatMessageForm
                      inputProps={{ placeholder: "New command...", disabled: chatLoading || showDiff }}
                    />
                  </div>
                </>
              )}
