...
const MenuButton = forwardRef<HTMLButtonElement, MenuButtonProps>(({ active, ...props }, ref) =>
  <Button size="xs" variant={"ghost"} {...props} className={clsx("", { "bg-primary text-primary-foreground hover:bg-primary/70": active }, props.className)} ref={ref} />);
...
export default function FloatingMenu(props: Omit<BubbleMenuProps, "children">) {
  ...
  return <TipTapBubbleMenu {...props}>
    <div className="flex gap-0.5 p-1 rounded bg-background border shadow-xl">
      ...
      <MenuButton onClick={() => editor.chain().focus().toggleBold().run()} active={editor.isActive("bold")}><BoldIcon /></MenuButton>
      <MenuButton onClick={() => editor.chain().focus().toggleItalic().run()} active={editor.isActive("italic")}><ItalicIcon /></MenuButton>
      <MenuButton onClick={() => editor.chain().focus().toggleStrike().run()} active={editor.isActive("strike")}><StrikethroughIcon /></MenuButton>
      <div className="w-px bg-primary-foreground"></div>

      <Popover>
        <PopoverTrigger asChild>
          <MenuButton active={editor.isActive("link")}><Link2Icon /></MenuButton>
        </PopoverTrigger>
